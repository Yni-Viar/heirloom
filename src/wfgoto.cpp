...
std::vector<PDNODE> FilterBySubtree(std::vector<PDNODE> const& parents, std::vector<PDNODE> const& children) {
    ...
    std::copy_if(
        std::cbegin(children), std::cend(children), std::back_inserter(results), [&parents](auto const& child) {
...
BOOL BuildDirectoryBagOValues(
    ...
    while (bFound) {
        ...
        if ((lfndta.fd.dwFileAttributes & ATTR_DIR) == 0 || ISDOTDIR(lfndta.fd.cFileName) ||
            lfndta.fd.cFileName[0] == CHAR_NULL) {
...
LRESULT APIENTRY GotoEditSubclassProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_GETDLGCODE:
            if (lParam) {
                ...
                if ((lpmsg->message == WM_KEYDOWN || lpmsg->message == WM_KEYUP) &&
                    (lpmsg->wParam == VK_DOWN || lpmsg->wParam == VK_UP || lpmsg->wParam == VK_HOME ||
                     lpmsg->wParam == VK_END || lpmsg->wParam == VK_PRIOR || lpmsg->wParam == VK_NEXT)) {
...
StartBuildingDirectoryTrie() {
    ...
    // Move/Copy things.
    //
    hThreadCopy = CreateThread(nullptr, 0L, BuildDirectoryTreeBagOValues, nullptr, 0L, &dwIgnore);
