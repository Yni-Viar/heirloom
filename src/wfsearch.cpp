...
INT SearchList(
    ...
    bLowercase = (wTextAttribs & TA_LOWERCASEALL) || ((wTextAttribs & TA_LOWERCASE) && !SearchInfo.bCasePreserved);
    ...
    if (!bFound && ERROR_FILE_NOT_FOUND != lfndta.err &&
        (bRoot ||
         ERROR_ACCESS_DENIED != lfndta.err && ERROR_PATH_NOT_FOUND != lfndta.err &&
             ERROR_INVALID_REPARSE_DATA != lfndta.err && ERROR_CANT_ACCESS_FILE != lfndta.err &&
             ERROR_SYMLINK_CLASS_DISABLED != lfndta.err && ERROR_INVALID_NAME != lfndta.err)) {
        SearchInfo.eStatus = _SEARCH_INFO::SEARCH_ERROR;
    ...
    while (bFound) {
        ...
        if (bFound && !ISDOTDIR(lfndta.fd.cFileName)) {
            ...
            SendMessage(hwndFrame, FS_SEARCHLINEINSERT, (WPARAM)&iFileCount, (LPARAM)lpxdta);
        }
...
SearchCleanup:
    ...
    while (bFound) {
        ...
        if (!ISDOTDIR(lfndta.fd.cFileName) && (lfndta.fd.dwFileAttributes & ATTR_DIR)) {
            ...
            //
            iRetVal =
                SearchList(hwndLB, pszNewPath, szFileSpec, bRecurse, bIncludeSubdirs, plpStart, iFileCount, FALSE);
...
INT FillSearchLB(HWND hwndLB, LPWSTR szSearchFileSpec, BOOL bRecurse, BOOL bIncludeSubdirs) {
    ...
    while (*lpszCurrentFileSpecEnd) {
        ...
        iRet = SearchList(hwndLB, szPathName, szWildCard, bRecurse, bIncludeSubdirs, &lpStart, iFileCount, TRUE);
...
#undef lpItem2
        ...
        case WM_FSC:
            ...
            //
            if (cDisableFSC || (hwnd != (HWND)SendMessage(hwndMDIClient, WM_MDIGETACTIVE, 0, 0L)) ||
        ...
        case FS_CHANGEDISPLAY:
            ...
            if (wParam == CD_VIEW || wParam == CD_SEARCHFONT) {
                ...
                if (CD_SEARCHFONT == wParam) {
                    ...
                    // but quick fix.
                    //
                    MemLinkToHead(SearchInfo.lpStart)->dwEntries = (DWORD)SendMessage(hwndLB, LB_GETCOUNT, 0, 0L);

                    maxExt = GetMaxExtent(hwndLB, SearchInfo.lpStart, FALSE);
                }

                FixTabsAndThings(hwndLB, (WORD*)GetWindowLongPtr(hwnd, GWL_TABARRAY), maxExt + dxText, 0, dwNewView);
        ...
        case WM_DRAWITEM: {
            ...
            if (maxExt > maxExtLast) {
                ...
                // Need to update tabs
                //
                FixTabsAndThings(SearchInfo.hwndLB, pwTabs, maxExt + dxText, 0, dwView);
...
INT_PTR CALLBACK SearchProgDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (uMsg) {
        case WM_INITDIALOG:
            ...
            // Initialize display
            //
            SendMessage(hwndFrame, FS_SEARCHUPDATE, SearchInfo.iDirsRead, SearchInfo.iFileCount);
            ...
            //
            if (!SearchInfo.hThread) {
                SearchInfo.hThread = CreateThread(
                    NULL,  // Security
                    0L,    // Stack Size
                    SearchDrive, NULL, 0L, &dwIgnore);
            }
...
VOID SearchEnd(VOID) {
    ...
    if (_SEARCH_INFO::SEARCH_MDICLOSE != SearchInfo.eStatus) {
        ...
        if (hwndMDIChild) {
            ShowWindow(
                hwndSearch, GetWindowLongPtr(hwndMDIChild, GWL_STYLE) & WS_MAXIMIZE ? SW_SHOWMAXIMIZED : SW_SHOWNORMAL);
        }
