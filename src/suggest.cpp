...
FormatError(BOOL bNullString, LPTSTR lpBuf, INT iSize, DWORD dwError) {
    ...
    if (!(dwError & DE_BEGIN) && !(pdwSuggest && pdwSuggest[1] & SUG_IGN_FORMATMESSAGE)) {
        ...
        if (ERROR_EXTENDED_ERROR == dwError) {
            ...
            if (WAITNET_LOADED) {
                ...
                WNetGetLastError(&dwErrorCode, lpBuf, iSize, szProvider, COUNTOF(szProvider));
            }
        ...
        do {
            dwNumChars = FormatMessage(
                ...
                dwError, wLangId, lpBuf, iSize * sizeof(lpBuf[0]), NULL);

            bTryAgain = !dwNumChars && MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL) != wLangId &&
