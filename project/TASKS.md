...
- [ ] Feature: On creating zips, `zip_close` takes a very long time. Use `zip_register_progress_callback_with_state` as described in the docs to add a percentage completion to the "Finalizing archive..." message, and use `zip_register_cancel_callback_with_state` to support canceling during the `zip_close` operation.
